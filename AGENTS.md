...
- **`@ag-ui/client` must stay deduped.** The root `package.json` pins it via
  `pnpm.overrides` to the exact version `@copilotkit/runtime` declares. Two copies
  ...
  `pnpm why -r @ag-ui/client` and update the override.
...
- Run `pnpm typecheck` before claiming anything works.
