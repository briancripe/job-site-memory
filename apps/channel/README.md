...
```bash
pnpm channel:setup --no-clipboard
```
...
If Slack is not connected, run pnpm channel:setup --no-clipboard
...
Run pnpm verify and document the live Slack checks separately.
```

## Verify and limits

Run `pnpm verify` for root/channel typechecks and offline tests. Live Slack delivery, Exa search, and model responses require your own accounts and should be documented separately from local tests.
