...
`pnpm verify` needs no `.env` or live credentials. The relevant app path
...
`@ag-ui/client`. The root `package.json` pins it via `pnpm.overrides`; check it still
matches what the runtime declares:

```bash
pnpm why -r @ag-ui/client     # every line should read the same version
```
...
- `pnpm verify` — retained workspace typechecks and offline tests
- `pnpm channel:status` — real doctor command for the Channel
