...
`pnpm verify` covers the retained root workspaces without credentials. Mobile is not a pnpm workspace member because React Native uses its own dependency versions; run its local install, tests, typecheck, and iOS/Android Metro export checks under `apps/mobile` when validating the mobile template.
