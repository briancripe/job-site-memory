...
Use the version Runtime declares (`pnpm why -r @ag-ui/client` shows both), then
...
see the `channels-setup` skill installed by `pnpm channel:setup --no-clipboard`.
...
question. Run `pnpm channel:setup --no-clipboard` from the repository root
